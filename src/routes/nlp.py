...
async def answer_query(request: Request, project_id: int, search_request: SearchRequest):
    ...
    answer, full_prompt, history = nlp_controller.answer_rag_question(
        project=project, 
        query=search_request.text, 
        limit=search_request.limit)
    ...
    if not answer:
        return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSignal.ERROR_RAG_ANSWER.value
                }
            )
    
    return JSONResponse(
        content={
            "signal": ResponseSignal.SUCCESS_RAG_ANSWER.value,
            "answer": answer,
